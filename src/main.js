...
function onboardingPage() {
  return `<main class="onboarding-page"><div class="onboarding-mark">✦</div><span class="eyebrow">NUSANTARA LEARNING</span><h1>Understand Indonesia, one connected story at a time.</h1><p class="onboarding-lede">A thoughtful Indonesia 101 journey that starts with the map, then connects history, people, belief, and everyday life.</p><div class="onboarding-points"><span>◈ Six guided classes</span><span>◷ 12–20 minute lessons</span><span>✦ Progress saved on this device</span></div><div class="onboarding-preview"><div><b>Inside every class</b><span>Story → visual → key ideas → vocabulary → recall quiz → resources</span></div><a href="#/class/what-is-indonesia">Preview class 1 →</a></div><form class="onboarding-form" data-onboarding-form><label>What should we call you?<input name="name" value="${escapeHtml(state.profile.name === 'Fredrick' ? '' : state.profile.name)}" placeholder="Your name" autocomplete="name" required/></label><label>What are you hoping to understand?<textarea name="goal" placeholder="For example: the people and history behind the places I visit">${escapeHtml(state.profile.goal === 'Understand Indonesia region by region' ? '' : state.profile.goal)}</textarea></label><button class="primary-button" type="submit">Begin your journey <span>→</span></button></form><button class="text-button guest-button" data-action="guest">Explore first, personalize later →</button><small class="onboarding-note">Private by design · no account or payment required</small></main>`;
}
