## 2026-09-21 08:41 WIB
- Improved spaced-review prioritization: due reviews are now sorted by their scheduled review date, so the oldest overdue concept appears first regardless of completion order. This preserves the existing local state shape and home resume pointer while making the dashboard queue and home review prompt more pedagogically coherent.
- Verification: strict TDD RED failed with the expected ordering mismatch; GREEN focused test passed, then `npm test` (20 passing), `npm run build` (Vite production build passed), `git diff --check` (passed), and local Vite HTTP smoke checks returned the app shell, manifest, and service worker endpoints. Full interactive browser automation remains unavailable because Chrome/Chromium and Playwright are not installed in this environment.
- Remaining risks: review/progress/reminder data remains device-local; browser notifications are best-effort and do not guarantee delivery while the app is closed; no cloud sync, auth, payments, or reliable external scheduler is included.

## 2026-09-21 08:08 WIB
- Improved the home resume path for spaced learning: a pure `getNextLearningAction` selector now prioritizes the oldest due review while preserving the learner’s current lesson pointer, and the home screen surfaces a responsive “Ready to recall” prompt linking to the review queue. No persisted state fields changed.
- Verification: strict TDD RED failed because the selector export was missing; GREEN `npm test` (19 passing), `npm run build` (Vite production build passed), `git diff --check` (passed), and live Vite smoke checks confirmed the app shell, module wiring, manifest, and service worker endpoints. Full interactive browser automation remains unavailable because Chrome/Chromium and Playwright are not installed in this environment.
- Remaining risks: review completion still happens from the dashboard; progress/history/reminders remain device-local; browser notifications are best-effort and do not guarantee delivery while the app is closed; no cloud sync, auth, payments, or reliable external scheduler is included.
...
- Remaining risks: levels are currently course-local and not persisted as separate state; profile/progress remain device-local; browser notifications are best-effort and do not guarantee delivery while the app is closed; no cloud sync, auth, payments, or reliable external scheduler is included.
